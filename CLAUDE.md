...
- **Code shape:** small, pure functions; do I/O at the edges. `finale_file_parser.ir.Score` is the
  canonical cross-format score representation; do not create a parallel one.
...
make check-full # the above plus the 35 corpus sweeps (~10 min) <- run before pushing
```
