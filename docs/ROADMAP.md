...
- [x] **`.mus` input** — `read_mus_document(path)` yields an `EnigmaDocument`; the stable facade's
      `read_score(path)` selects that reader by content, so `to_musicxml(read_score(path))` exports
      a legacy `.mus`. Validated IR
